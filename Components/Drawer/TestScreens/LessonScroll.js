...
function LessonScroll(props) {
    ...
    return (
        <View style={{ backgroundColor: 'cyan', height: Dimensions.get('screen').height, width: Dimensions.get('screen').width }}>
            {console.log("State in lessonscroll = " + JSON.stringify(state.lesson))}
            {state.lesson == null? 
            <ActivityIndicator size="large" color="cyan"
             style={{ alignSelf: 'center', transform:[{scale:2}], marginTop: 150, width: 200, height: 200}}/>: 
             <LessonDetails lesson={state.lesson} />
             }
        </View>
    )
}

...
function CourseDetails(props) {
    ...
    return (
        <View style={{ display: 'flex', height: Dimensions.get('screen').height, flexDirection: 'column', justifyContent: 'space-around', alignContent: 'space-around', flex: 4 }}>
            ...
            <Text style={{ fontFamily: 'Sukarblack', fontSize: 20, textAlign: 'center', textDecorationLine: 'underline' }}>{state.course.name}</Text>
            <Text style={{ fontFamily: 'Sukarblack', fontSize: 15, padding: 20, textAlign: 'right' }}>{state.course.description}</Text>
            </View>
            ...
            style={{ 
             ...
             >
                 {console.log("Lessons = " + JSON.stringify(state.lessons))}
                 {console.log(JSON.stringify(state.course))}
                <Text style={{ textAlign: 'center', fontFamily: 'Sukarblack', fontSize: 20, color: 'purple' }}>
                    الاشتراك الان
                </Text>
            </TouchableOpacity>
